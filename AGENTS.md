...
- Game systems (constants in `src/main.odin`): player command bases exist only on Earth (`base_counts = {1,0,0}`); `start_base_construction`/`queue_unit` are gated on `selected_planet == 0`, and the inspector renders base/production/queue sections only for Earth (`draw_earth_inspector` vs `draw_outpost_inspector`). Jupiter starts enemy-occupied (garrison fighters/miners + 20 HP `enemy_base_hp`); a planet is liberated when its base hits 0, which the combat rules only allow after every enemy drone there is dead. Enemy waves: first at 180s then every 120s, target Mars until Jupiter is liberated, then a seeded random Mars/Jupiter (`rl.SetRandomValue`, deterministic in tests via `rl.SetRandomSeed`). Base construction: Earth only, 200 minerals + 5 player mining drones present (`.MINING` at Earth); they switch to `.CONSTRUCTING` for 60s (MPS stops), then resume. Mining is Earth round-trip based: transit out, mine `MINING_DURATION` (3s), transit back, deposit `DEPOSIT_DURATION` (0.5s) at `MINING_TRANSIT_SPEED` (1.75); `planet_mps` = effective miners * `mining_rate` / (3.5 + 2*dist(p,Earth)/1.75), so MPS falls with distance. Mining drones hold `.IDLE` at an occupied planet until liberated, then resume. Transit speeds: combat 2.5/s, mining 1.75/s (25% of the old pace); transit and guarding fighters render `ceil(count/10)` cubes per side/group.
