...
- Game systems (constants in `src/main.odin`): Mars and Jupiter start enemy-occupied (garrison fighters/miners + per-planet `enemy_base_hp`); a planet is liberated when its base hits 0, which the combat rules only allow after every enemy drone there is dead. Enemy waves: first at 180s then every 120s, target Mars until Jupiter is liberated, then a seeded random Mars/Jupiter (`rl.SetRandomValue`, deterministic in tests via `rl.SetRandomSeed`). Base construction: needs liberated planet + 200 minerals + 5 player mining drones present (`.MINING` at the planet); they switch to `.CONSTRUCTING` for 60s (MPS stops), then resume. Transit speeds: combat 2.5/s, mining 1.75/s (25% of the old pace); transit and guarding fighters render `ceil(count/10)` cubes per side/group.
